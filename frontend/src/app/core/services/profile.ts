...
import { ProfileData, SkillData } from '../models/profile';
...
export class ProfileService {
  ...
  private _profile = signal<Omit<ProfileData, 'age'>>({
    fullname_profile: '',
    photo_profile: '',
    phone_profile: '',
  ...
  private profile$: Observable<Omit<ProfileData, 'age'>> = this.http.get<Omit<ProfileData, 'age'>>(`/assets/data/profile.json`).pipe(
    tap(data => { // Only update the signal if the API returns valid data
      if (data) {
        console.log('Profile data fetched from API:', data);
    ...
    catchError(err => {
      ...
      return of(this._profile()); // On error, return the current (initial) value
    })
  );
   ...
   */
  public updateProfile(updatedProfile: Omit<ProfileData, 'age'>): Observable<Omit<ProfileData, 'age'>> {
